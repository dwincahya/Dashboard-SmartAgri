...
const SistemSpraying = () => {
  const [mode, setMode] = useState("manual"); // manual, scheduled
  const [isSprayerOn, setIsSprayerOn] = useState(false);
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      {/* Manual Mode: Sprayer Switch */}
      {/* Manual Mode: Ventilation Switch */}
      {mode === "manual" && (
        <div className="mb-4">          
          <label class="inline-flex items-center cursor-pointer">
            ...
            <span class="ms-3 text-sm font-medium dark:text-gray-300">Kontrol Ventilasi</span>
          </label>
      ...
      {mode === "scheduled" && (
        <div className="mb-4">
          <label className="block mb-2 text-gray-600">Waktu Spraying</label>
          <input
            type="time"
            className="w-full p-2 border rounded-lg"
            value={scheduleTime}
